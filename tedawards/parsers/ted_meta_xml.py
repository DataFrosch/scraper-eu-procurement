...
class TedMetaXmlParser(BaseParser):
    ...
    def _parse_meta_xml_zip(self, zip_path: Path) -> Optional[TedParserResultModel]:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                if award_records:
                    logger.debug(
                        f"Found {len(award_records)} award records in {zip_path.name}"
                    )
    ...
    def _convert_meta_xml_to_standard_format(
        ...
        try:
            ...
            codifdata = (
                doc_elem.xpath(".//codifdata")[0]
                if doc_elem.xpath(".//codifdata")
                else None
            )
            ...
            nodocojs = (
                codifdata.xpath("./nodocojs/text()")[0]
                if codifdata.xpath("./nodocojs/text()")
                else doc_id
            )
            datedisp = (
                codifdata.xpath("./datedisp/text()")[0]
                if codifdata.xpath("./datedisp/text()")
                else ""
            )
            isocountry = (
                codifdata.xpath("./isocountry/text()")[0]
                if codifdata.xpath("./isocountry/text()")
                else ""
            )
            ...
            datepub = ""
            refojs = (
                doc_elem.xpath(".//refojs")[0] if doc_elem.xpath(".//refojs") else None
            )
            if refojs is not None:
                datepub = (
                    refojs.xpath("./datepub/text()")[0]
                    if refojs.xpath("./datepub/text()")
                    else ""
                )
            ...
            full_doc_id = (
                f"meta-{nodocojs}-{datepub}" if datepub else f"meta-{nodocojs}"
            )
