...
class TedInternalOjsParser(BaseParser):
    ...
    def can_parse(self, file_path: Path) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Error checking if {file_path.name} is INTERNAL_OJS format: {e}"
            )
            return False
    ...
    def _extract_document_info(self, root, xml_file: Path) -> Optional[dict]:
        ...
        try:
            ...
            if deletion_date_str and len(deletion_date_str) == 8:
                try:
                    deletion_date = datetime.strptime(
                        deletion_date_str, "%Y%m%d"
                    ).date()
                except ValueError:
                    pass
        ...
        try:
            ...
            for award_elem in award_elements:
                ...
                for contractor_elem in contractor_elems:
                    contact_data = (
                        contractor_elem.xpath(
                            ".//CONTACT_DATA_WITHOUT_RESPONSIBLE_NAME"
                        )[0]
                        if contractor_elem.xpath(
                            ".//CONTACT_DATA_WITHOUT_RESPONSIBLE_NAME"
                        )
                        else None
                    )
