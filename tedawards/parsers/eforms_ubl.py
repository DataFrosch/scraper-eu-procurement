...
class EFormsUBLParser(BaseParser):
    ...
    def _extract_document_data(self, root, ns, xml_file: Path) -> Optional[Dict]:
        ...
        try:
            ...
            pub_date_elem = (
                root.xpath(".//efac:Publication/efbc:PublicationDate", namespaces=ns)
                or root.xpath(".//cbc:IssueDate", namespaces=ns)
                or root.xpath(".//efac:SettledContract/cbc:IssueDate", namespaces=ns)
                or root.xpath(".//cac:ContractAwardNotice/cbc:IssueDate", namespaces=ns)
            )
            ...
            try:
                ...
                logger.error(
                    f"Invalid publication date format in {xml_file}: {pub_date_elem[0].text}. Expected ISO date format. Error: {e}"
                )
                raise

            # Extract sender country
            countries = root.xpath(
                ".//cac:Country/cbc:IdentificationCode/text()", namespaces=ns
            )
            ...
            day_of_year = (
                pub_date.timetuple().tm_yday if hasattr(pub_date, "timetuple") else 1
            )
    ...
    def _extract_contracting_body(self, root, ns) -> Optional[Dict]:
        ...
        try:
            ...
            if not contracting_party_id:
                ...
                orgs = root.xpath(
                    ".//efac:Organizations/efac:Organization", namespaces=ns
                )
                if orgs:
                ...
                for org in orgs:
                    ...
                    if company is not None:
                        ...
                        org_id = (
                            org_id_elem[0].text
                            if org_id_elem and org_id_elem[0].text
                            else None
                        )
            ...
            address_elem = contracting_body.xpath(
                ".//cac:PostalAddress/cbc:StreetName", namespaces=ns
            )
            town_elem = contracting_body.xpath(
                ".//cac:PostalAddress/cbc:CityName", namespaces=ns
            )
            postal_elem = contracting_body.xpath(
                ".//cac:PostalAddress/cbc:PostalZone", namespaces=ns
            )
            country_elem = contracting_body.xpath(
                ".//cac:PostalAddress/cac:Country/cbc:IdentificationCode", namespaces=ns
            )
            ...
            return {
                "official_name": name_elem[0].text
                if (name_elem and name_elem[0].text)
                else "",
                "address": address_elem[0].text
                if (address_elem and address_elem[0].text)
                else None,
                "town": town_elem[0].text
                if (town_elem and town_elem[0].text)
                else None,
                "postal_code": postal_elem[0].text
                if (postal_elem and postal_elem[0].text)
                else None,
                "country_code": country_elem[0].text
                if (country_elem and country_elem[0].text)
                else None,
                "nuts_code": None,  # TODO: Extract NUTS if available
                "contact_point": "",
                "phone": phone_elem[0].text
                if (phone_elem and phone_elem[0].text)
                else None,
                "email": email_elem[0].text
                if (email_elem and email_elem[0].text)
                else None,
                "fax": "",
                "url_general": url_elem[0].text
                if (url_elem and url_elem[0].text)
                else None,
                "url_buyer": "",
                "authority_type_code": "",
                "main_activity_code": "",
    ...
    def _extract_contract(self, root, ns) -> Optional[Dict]:
        ...
        try:
            ...
            ref_elem = root.xpath(
                ".//efac:SettledContract/efac:ContractReference/cbc:ID", namespaces=ns
            )
            ref_number = ref_elem[0].text if (ref_elem and ref_elem[0].text) else None

            # Get total value
            total_amount = root.xpath(
                ".//efac:NoticeResult/cbc:TotalAmount", namespaces=ns
            )
            total_value = None
            total_currency = ""
            ...
            nature_elem = root.xpath(
                ".//cac:ProcurementProject/cbc:ProcurementTypeCode", namespaces=ns
            )
            contract_nature_code = (
                nature_elem[0].text if (nature_elem and nature_elem[0].text) else None
            )

            # Extract procedure type
            proc_elem = root.xpath(
                ".//cac:TenderingProcess/cbc:ProcedureCode", namespaces=ns
            )
            procedure_type_code = (
                proc_elem[0].text if (proc_elem and proc_elem[0].text) else None
            )
        ...
        try:
            ...
            for lot_result in lot_results:
                ...
                conclusion_date_elem = root.xpath(
                    ".//efac:SettledContract/cbc:IssueDate", namespaces=ns
                )
                ...
                if conclusion_date_elem and conclusion_date_elem[0].text:
                    try:
                        ...
                        logger.error(
                            f"Invalid conclusion date format: {conclusion_date_elem[0].text}. Expected ISO date format. Error: {e}"
                        )
                        raise
                ...
                award_title_elem = root.xpath(
                    ".//efac:SettledContract/cbc:Title", namespaces=ns
                )
                award_title = (
                    award_title_elem[0].text
                    if (award_title_elem and award_title_elem[0].text)
                    else None
                )
                ...
                contract_number = (
                    contract_num_elem[0].text
                    if (contract_num_elem and contract_num_elem[0].text)
                    else None
                )
        ...
        try:
            ...
            for party in tenderer_parties:
                tenderer_ids = party.xpath(
                    ".//efac:Tenderer/cbc:ID/text()", namespaces=ns
                )
